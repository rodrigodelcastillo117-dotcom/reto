-- ============================================================================
-- SEMANTIC ROLLBACK (PRIMARIO, NO CASCADE) -- ISS-003/009/009B
-- Restaura COMPORTAMIENTO pre-deploy preservando disponibilidad: CREATE OR REPLACE
-- (sin DROP), conservando columnas aditivas como inertes (NULL) por compatibilidad.
-- RESTORE_BEHAVIOR + PRESERVE_AVAILABILITY + NO_CASCADE. NO EJECUTAR salvo incidente.
-- Orden: analisis_completo -> v_mejores_picks_mlb -> v_pick_canonico.
-- ============================================================================
BEGIN;
SET LOCAL lock_timeout = '3s';
SET LOCAL statement_timeout = '60s';

-- 1) analisis_completo(text) -> def previa (CREATE OR REPLACE, sin drop)
...
-- 2) v_mejores_picks_mlb -> comportamiento previo + cols aditivas inertes (NO DROP)
CREATE OR REPLACE VIEW public.v_mejores_picks_mlb AS
  SELECT sub.*, NULL::boolean AS economically_eligible, NULL::text AS reason_code
  FROM (
 WITH picks AS (
...
-- 3) v_pick_canonico -> comportamiento previo + es_pick_reason inerte (NO DROP, NO CASCADE)
CREATE OR REPLACE VIEW public.v_pick_canonico AS
  SELECT sub.*, NULL::text AS es_pick_reason
  FROM (
