-- ISS154b — corrected release P0 Parlay cutover.
-- Supersedes iss154_parlay_fast_pool_global_preto.sql (syntax-only failure; never deployed).
...
select
  ...
  case when g.p_reto_pct >= 65 then 'P_RETO_HIGH' when g.p_reto_pct >= 58 then 'P_RETO_MEDIUM' else 'P_RETO_CAUTION' end as confidence_band,
  g.model_version,
...
left join lateral (
  select
    ...
    x.bookmaker,
    case when x.home_ml>1 and x.draw_ml>1 and x.away_ml>1 then
      case
...
with base as (
  ...
  select b.*, jsonb_build_object(
    ...
    'separacion_modelo_pp',null,'calibracion_confiable',true,'muestra_calibracion',null,
    'contexto_mercado',jsonb_build_object('momio_mercado',b.selected_decimal_odds,'casa',b.odds_bookmaker),
    'es_lock',false,'espn_event_id',b.espn_event_id,'rank_global',b.rank_global
  ) as j from base b
), top3 as (select * from fila order by p_reto_pct desc,kickoff,espn_event_id limit 3),
alta as (select * from fila where p_reto_pct>=65 order by p_reto_pct desc,kickoff,espn_event_id limit 5),
top6 as (select * from fila order by p_reto_pct desc,kickoff,espn_event_id limit 6)
select jsonb_build_object(
  'generado_at',now(),'ventana_horas',least(greatest(p_ventana_horas,1),48),
...
begin
  ...
  if v_n<v_target then return jsonb_build_object('ok',true,'status','NO_QUALITY_PARLAY','profile',v_profile,
    'target_legs',v_target,'eligible_diversified_legs',v_n,'legs',v_legs,'thresholds',jsonb_build_object('min_p_reto_pct',v_min_p),
