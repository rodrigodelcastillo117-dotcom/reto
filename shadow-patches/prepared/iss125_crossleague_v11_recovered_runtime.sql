-- ISS125 — RECOVER CROSSLEAGUE v1_1 RUNTIME FROM EXECUTED SUPABASE HISTORY
-- STAGED ONLY. NO PRODUCTION CUTOVER.
--
-- Source of truth for this file: exact executed migration history recovered from
-- soccer-final-branch-gate (rotbmkcqvaeqmfdormfp):
--   20260911160312 iss052_soccer_crossleague_v11_convergence
--   20260911160334 iss052a_soccer_crossleague_v11_runtime_fix
--   20260911160352 iss052b_soccer_v11_idempotent_builder
--   20260911160402 iss052c_soccer_quarter_line_failclose
-- This captures the previously unversioned runtime that independently rebuilt the
-- six owner UCL fixtures 6/6 under crossleague_v1_1.
...
create table if not exists v2.soccer_domestic_observation (
  team_espn_id text not null, provider text not null, provider_team_id text,
  provider_fixture_id text not null, domestic_league_id integer not null,
  domestic_league_name text, kickoff timestamptz not null, gf integer not null,
  ga integer not null, loaded_at timestamptz not null,
  metadata jsonb not null default '{}'::jsonb,
  primary key (team_espn_id, provider, provider_fixture_id)
);

create table if not exists v2.crossleague_model_registry (
  model_version text not null, training_cutoff timestamptz not null,
  a0 numeric not null, batt numeric not null, bdef numeric not null,
  home_adv numeric not null, rho numeric not null, ridge numeric not null,
  ref_league_id integer, sample_floor_domestic integer not null,
  validation_status text not null, validation_evidence jsonb not null,
  source_commit text, created_at timestamptz not null default now(),
  primary key (model_version, training_cutoff)
);

create table if not exists v2.crossleague_league_strength (
  model_version text not null, training_cutoff timestamptz not null,
  league_id integer not null, league_name text, phi numeric not null,
  n_cross integer not null, servable boolean not null, config_hash text not null,
...
create table if not exists v2.crossleague_strength_seal (
  model_version text not null, training_cutoff timestamptz not null,
  league_count integer not null, config_hash text not null, snapshot_hash text not null,
  sealed_at timestamptz not null default now(),
  primary key (model_version, training_cutoff)
);

create table if not exists v2.crossleague_competition_policy (
  competition_id text primary key, model_version text not null, status text not null,
...
values (
 'crossleague_v1_1','2026-09-08 19:00:00+00',-0.2214,0.6057,0.3896,0.3568,
 0.0705,10.0,39,15,'OOS_VALIDATED_PROD_EXTENDED',
...
insert into v2.crossleague_competition_policy
(competition_id,model_version,status,evidence,approved_at)
values
('uefa_champions_league','crossleague_v1_1','PROD_APPROVED',
 '{"basis":"UCL OOS n=207; extended league-strength holdouts passed"}'::jsonb,now()),
('uefa_europa_league','crossleague_v1_1','PROD_APPROVED',
 '{"basis":"UEL OOS n=136; cross-league OOS improvement"}'::jsonb,now())
on conflict (competition_id) do update
set model_version=excluded.model_version,status=excluded.status,evidence=excluded.evidence,
    approved_at=coalesce(v2.crossleague_competition_policy.approved_at,excluded.approved_at),
    updated_at=now();

insert into v2.crossleague_strength_seal
(model_version,training_cutoff,league_count,config_hash,snapshot_hash)
select 'crossleague_v1_1','2026-09-08 19:00:00+00'::timestamptz,count(*),max(config_hash),
       md5(string_agg(concat_ws(':',league_id::text,phi::text,n_cross::text,
                                servable::text,config_hash),'|' order by league_id))
from v2.crossleague_league_strength
where model_version='crossleague_v1_1'
  and training_cutoff='2026-09-08 19:00:00+00'
on conflict (model_version,training_cutoff) do nothing;

create or replace function v2.fn_crossleague_strength_integrity(
...
returns boolean language sql stable as $$
  select exists(
    ...
    where s.model_version=p_model_version and s.training_cutoff=p_cutoff
      and s.league_count=(select count(*) from v2.crossleague_league_strength x
                          where x.model_version=p_model_version and x.training_cutoff=p_cutoff)
      and s.config_hash=(select max(config_hash) from v2.crossleague_league_strength x
                         where x.model_version=p_model_version and x.training_cutoff=p_cutoff)
      and s.snapshot_hash=(select md5(string_agg(concat_ws(':',league_id::text,phi::text,
                           n_cross::text,servable::text,config_hash),'|' order by league_id))
...
returns table(n int,gf numeric,ga numeric,domestic_league_id int,
              data_asof timestamptz,loaded_asof timestamptz,source_provider text)
...
with preferred as (
  ...
  where c.deporte='soccer' and c.espn_team_id=p_team_espn_id
  ...
  where (d.home_espn_id=p_team_espn_id or d.away_espn_id=p_team_espn_id)
    ...
    and (d.liga_id=(select liga_id from preferred)
         or ((select liga_id from preferred) is null and lm.espn_endpoint ~ '\.1$'))
  group by d.liga_id
), espn_best as (
  select n,gf,ga,liga_id,data_asof,loaded_asof,'ESPN'::text source_provider
  from espn_groups
  order by case when liga_id=(select liga_id from preferred) then 0 else 1 end,
           n desc,data_asof desc limit 1
), api_groups as (
  select count(*)::int n,avg(o.gf)::numeric gf,avg(o.ga)::numeric ga,
         o.domestic_league_id liga_id,max(o.kickoff) data_asof,
         max(o.loaded_at) loaded_asof,max(o.provider)::text source_provider
  from v2.soccer_domestic_observation o
  where o.team_espn_id=p_team_espn_id and o.kickoff<p_decision_time
    and o.kickoff>=p_decision_time-make_interval(days=>p_window_days)
    and o.loaded_at<=p_decision_time
...
from candidates
order by n desc,case when source_provider='ESPN' then 0 else 1 end,data_asof desc
limit 1;
$$;

-- Final runtime-fixed v1_1 emitter (iss052a).
...
begin
  ...
  if h.data_asof>p_decision_time or a.data_asof>p_decision_time
     or h.loaded_asof>p_decision_time or a.loaded_asof>p_decision_time then
  ...
  if ph.phi is null or pa.phi is null or ph.servable is distinct from true
     or pa.servable is distinct from true then
  ...
  lh:=exp(least(m.a0+m.batt*ln(greatest(h.gf,0.05))+m.bdef*ln(greatest(a.ga,0.05))
                +m.home_adv+(ph.phi-pa.phi),2.5));
  la:=exp(least(m.a0+m.batt*ln(greatest(a.gf,0.05))+m.bdef*ln(greatest(h.ga,0.05))
                +(pa.phi-ph.phi),2.5));
  ...
  provenance:=jsonb_build_object(
    'engine','crossleague_v1_1','source_commit',m.source_commit,
...
-- Final idempotent builder (iss052b).
create or replace function v2.build_soccer_prediction_v2_staged(
  p_decision_time timestamptz,p_enforce_availability boolean default true,
  p_mapping_version text default null)
returns integer language plpgsql as $$
declare n int;r record;x record;v_sid uuid;v_has_canonical boolean;
begin
  n:=v2.build_soccer_prediction_v2_staged_v1_legacy(
       p_decision_time,p_enforce_availability,p_mapping_version);
  ...
  for r in
    select distinct a.espn_event_id,a.home_espn_id,a.away_espn_id,a.liga_id,
           ...
           (select s.over_line from v2.soccer_prediction_v2_staged s
             ...
             order by case when s.model_version='crossleague_v1_1' then 0 else 1 end,
                      s.built_at desc limit 1) over_line
    from public.agenda_espn a
    join v2.competition_catalog c
      on c.sport='soccer' and c.provider='espn'
     and c.provider_competition_id=a.liga_id::text
    join v2.crossleague_competition_policy cp
      on cp.competition_id=c.competition_id and cp.status='PROD_APPROVED'
     and cp.model_version='crossleague_v1_1'
    where a.deporte='soccer' and a.fecha>p_decision_time
  loop
    select * into x
    from v2.fn_crossleague_v11_staged(
      r.home_espn_id,r.away_espn_id,p_decision_time,
      r.catalog_competition_id,r.over_line);

    select exists(
      select 1 from v2.soccer_prediction_v2_staged s
    ...
    if x.gf_home is not null and x.gf_away is not null then
      ...
      values(
        r.espn_event_id,p_decision_time,x.gf_home,x.ga_home,x.gf_away,x.ga_away,
      ...
      select feature_snapshot_id into v_sid
      from v2.feature_snapshot
    ...
    update v2.soccer_prediction_v2_staged s set
      ...
      temporal_safe=(x.model_status='READY_UNVALIDATED' and x.data_asof<=p_decision_time
                     and x.loaded_asof<=p_decision_time),
      ...
      provenance=coalesce(x.provenance,'{}'::jsonb)
                 ||jsonb_build_object('model_route','CROSSLEAGUE','canonical_brain',true),
...
where s.model_version<>'crossleague_v1_1'
   or coalesce(s.provenance->>'engine','')<>'crossleague_v1_1';

-- iss052c: unsupported quarter totals fail closed; half and whole only.
