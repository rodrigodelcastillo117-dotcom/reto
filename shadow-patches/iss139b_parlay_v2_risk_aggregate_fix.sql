-- ISS139b — isolated fix for Parlay V2 risk aggregation.
-- No legacy function/table is modified.
...
begin
  ...
  with rr as (
    select x,
           (x->>'p_reto_pct')::numeric as p,
           sum(100-(x->>'p_reto_pct')::numeric) over() as total_failure
  ...
  select jsonb_agg(jsonb_build_object(
           ...
           'pick',x->>'pick',
           'p_reto_pct',p,
           'failure_probability_pct',round(100-p,2),
           'risk_share_pct',round(100*(100-p)/nullif(total_failure,0),2)
         ) order by p asc),
         (jsonb_agg(jsonb_build_object(
           'espn_event_id',x->>'espn_event_id','pick',x->>'pick','p_reto_pct',p
         ) order by p asc))->0
