-- ============================================================================
-- ISS-003/009/009B — ARTEFACTO V2 (CORRIGE DEFECTO POSICIONAL DE V1)
-- ============================================================================
-- V1 (sha 57b7a4077247e5e814aa9e4ce7e0ad369dc11975a8bff7ea28083c3ffedd4cad) queda
-- RETIRED / DO_NOT_DEPLOY: su Parte 2 insertaba `economically_eligible` y
-- `reason_code` en las posiciones 15/16, desplazando `nivel` de 15 a 17.
-- PostgreSQL rechaza eso en CREATE OR REPLACE VIEW:
--   ERROR: cannot change name of view column "nivel" to "economically_eligible"
-- Defecto determinista: V1 nunca fue ejecutable en ningún entorno.
--
-- ÚNICO CAMBIO DE V2 vs V1: en la Parte 2, las dos columnas nuevas se mueven al
-- FINAL del SELECT (posiciones 23 y 24), después de `pick_es_favorito`. Las 22
-- columnas preexistentes conservan nombre, tipo y posición. Las expresiones se
-- mueven byte-a-byte: no se reescribe ninguna, no cambia P, EV, Kelly, modelos,
-- thresholds ni autoridad económica. Parte 1, Parte 3 y Parte 4 son idénticas a V1
-- (verificable con: diff <(sed -n '35,$p' V1) <(sed -n '<offset>,$p' V2)).
-- La Parte 1 ya era ordinal-correcta en V1 (es_pick_reason additiva en #44).
-- ============================================================================
