...
def tab_historial(apodo: str, df: pd.DataFrame):
    ...
    # ── HISTORIAL: PENDIENTES vs RESUELTOS ────────────────────
    
    # Separar pendientes y resueltos
    pending_df = filt[filt["resultado"] == "pendiente"]
    resolved_df = filt[filt["resultado"] != "pendiente"]
    
    # ════════════════════════════════════════════════════════════════
    # ⏳ PICKS PENDIENTES (PEQUEÑOS, COMPACTOS)
    # ════════════════════════════════════════════════════════════════
    if not pending_df.empty:
        st.write("### ⏳ PICKS PENDIENTES")
        ...
        for idx, row in pending_df.iterrows():
            ...
            # Layout compacto: 5 columnas
            col1, col2, col3, col4, col5, col6 = st.columns([0.5, 2, 1.5, 1, 1.2, 0.3])
            with col1:
                st.caption(sp_ico)
            with col2:
                st.caption(f"**{partido[:40]}**")
            with col3:
                st.caption(f"{pick_d[:15]}")
            with col4:
                st.caption(f"@{momio}x")
            with col5:
                st.caption(f"${apuesta:,.0f} {pot}")
            with col6:
                if st.button("🗑", key=f"del_pending_{idx}", use_container_width=True):
    ...
    # ════════════════════════════════════════════════════════════════
    # ✅ PICKS RESUELTOS (PEQUEÑOS, AGRUPADOS POR DEPORTE, COLAPSADOS)
    # ════════════════════════════════════════════════════════════════
    if not resolved_df.empty:
        st.write("### ✅ PICKS RESUELTOS")
        ...
        # Mostrar cada deporte en expander colapsado
        for deporte in sorted(by_sport.keys()):
            ...
            nulls = sum(1 for _, r in picks_list if str(r.get("resultado","")) == "nulo")
            
            label = f"{sp_ico} {deporte.upper()} · {len(picks_list)} · {wins}✅ {losses}❌ {nulls}➖"
            with st.expander(label, expanded=False):
                for idx, row in picks_list:
                    ...
                    c1, c2, c3, c4, c5, c6 = st.columns([0.8, 2, 1.5, 0.8, 1.2, 0.3])
                    with c1:
                        st.caption(f"{fd}")
                    with c2:
                        st.caption(f"{partido[:35]}")
                    with c3:
                        st.caption(f"{pick_d[:12]}")
                    with c4:
                        st.markdown(f'<span style="color:{gc};font-weight:700;font-size:0.85rem">{ico}</span>', unsafe_allow_html=True)
                    with c5:
                        st.markdown(f'<span style="color:{gc};font-weight:700;font-size:0.85rem">{gan:+.0f}</span>', unsafe_allow_html=True)
                    with c6:
                        if st.button("🗑", key=f"del_resolved_{idx}", use_container_width=True):
                            if delete_pick(apodo, idx):
                                st.session_state.pop("df_picks", None)
                                st.rerun()
