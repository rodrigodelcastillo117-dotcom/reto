...
def tab_historial(apodo: str, df: pd.DataFrame):
    ...
    if not pending.empty:
        ...
        for idx, row in pending.iterrows():
            ...
            with col_card:
                ...
                with r1:
                    st.markdown(f'<div style="font-size:2rem;padding-top:2px;text-align:center">{sp_ico}</div>', unsafe_allow_html=True)
                with r2:
                    st.markdown(f'<div style="font-size:.95rem;font-weight:700;color:#EEEEF5">{format_partido_para_display(partido, deporte)}</div><div style="font-size:.62rem;color:#8888AA;margin-top:1px">{liga}{" · " + mercado if mercado else ""}</div>', unsafe_allow_html=True)
                with r3:
                    gs = f"+${gan:,.2f}" if gan>0 else f"-${abs(gan):,.2f}" if gan<0 else pot or "⏳"
                    st.markdown(f'<div style="text-align:right"><div style="font-family:\'Bebas Neue\',sans-serif;font-size:1.4rem;color:{gc};line-height:1">{gs}</div><div style="font-size:.58rem;color:#8888AA">{"@" + str(momio) + "x  ·  " if momio > 0 else ""}${apuesta:,.0f}</div></div>', unsafe_allow_html=True)

                r4, r5 = st.columns([6, 4])
                with r4:
                    st.markdown(f'<div style="background:rgba(0,255,209,.07);border-left:3px solid var(--neon2);padding:4px 10px;border-radius:0 6px 6px 0;margin-top:6px"><span style="font-size:.65rem;color:#8888AA">🎯 PICK  </span><span style="font-size:.82rem;font-weight:700;color:var(--neon2)">{pick_d}</span></div>', unsafe_allow_html=True)
                with r5:
                    st.markdown(f'<div style="text-align:right;padding-top:10px"><span style="font-size:.7rem;font-weight:700;color:{clr}">{ico} {res.upper()}</span>  <span style="font-size:.58rem;color:#8888AA">{fd}</span></div>', unsafe_allow_html=True)
    ...
    # RESUELTOS: Pequeño, agrupado por deporte
    if not resolved.empty:
        st.write("### ✅ RESUELTOS")
        by_sport = {}
        ...
        for deporte in sorted(by_sport.keys()):
            ...
            with st.expander(f"{sp_ico} {deporte.upper()} · {len(picks_list)} · {wins}W-{losses}L", expanded=False):
                for idx, row in picks_list:
                    ...
                    pick_d = str(row.get("pick_desc","")) or "—"
                    gc = "#00FF88" if gan>0 else "#FF2D55" if gan<0 else "#8888AA"
                    
                    c1, c2, c3, c4, c5 = st.columns([1.5, 2.5, 1.5, 1, 0.4])
                    with c1:
                        st.caption(f"{fd}")
                    with c2:
                        st.caption(f"{format_partido_para_display(partido, deporte)[:30]}")
                    with c3:
                        st.caption(f"{pick_d[:12]}")
                    with c4:
                        st.markdown(f'<span style="color:{gc};font-weight:700">{ico} {gan:+.0f}</span>', unsafe_allow_html=True)
                    with c5:
                        if st.button("🗑", key=f"del_res_{idx}"):
                            if delete_pick(apodo, idx):
                                st.session_state.pop("df_picks", None)
                                st.rerun()
